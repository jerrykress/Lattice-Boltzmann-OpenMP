...
float av_velocity(const t_param params, t_speed *cells, float *tot_u, int *obstacles, t_ocl ocl);

/* calculate Reynolds number */
float calc_reynolds(const t_param params, t_speed *cells, float *tot_u, int *obstacles, t_ocl ocl);
...
int initialise(const char *paramfile, const char *obstaclefile, t_param *params, t_speed **cells_ptr, t_speed **tmp_cells_ptr, int **obstacles_ptr, float **tot_u, float **av_vels_ptr, t_ocl *ocl)
{
  ...
  *av_vels_ptr = (float *)malloc(sizeof(float) * params->maxIters);
  ...
  ocl->tot_u = clCreateBuffer(ocl->context, CL_MEM_READ_WRITE, sizeof(cl_float) * params->nx * params->ny, NULL, &err);
  checkError(err, "creating tot_u buffer", __LINE__);

  return EXIT_SUCCESS;
}

int finalise(const t_param *params, t_speed **cells_ptr, t_speed **tmp_cells_ptr, int **obstacles_ptr, float **tot_u, float **av_vels_ptr, t_ocl ocl)
{
  /*
